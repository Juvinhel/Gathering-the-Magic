namespace Views.Repository.Search
{
    export class TypeInputElement extends HTMLElement
    {
        ...
        {
            return [
                <input class="value-input" type="text" placeholder="Input type" onkeyup={ this.keyup.bind(this) } onfocus={ this.focused.bind(this) } />,
                <drop-down onopen={ this.opened.bind(this) } autoAttach={ true }>
                    <ul class="types-list" onkeyup={ this.keyup.bind(this) } >{ this.typeList() }</ul>
                </drop-down>
            ];
        }
        ...
        private * typeList()
        {
            ...
            {
                yield <li class="type-category">{ entry[0] }</li>;
                for (const type of entry[1])
                    yield <li onclick={ this.clicked.bind(this) }>{ type }</li>;
            }
        }
